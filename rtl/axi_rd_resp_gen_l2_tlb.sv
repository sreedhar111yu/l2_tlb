...
module axi_rd_resp_gen_l2_tlb (
  ...
  // 600MHz FIX: Zero Bubble Forwarding
  // FIFO can send data if we are empty, OR if AXI is consuming current data
  assign l2_tlb_resp_ready_o = !resp_valid_q || axi_rready_i;
  ...
  always_ff @(posedge clk_i or negedge rstn_i) begin
    if (!rstn_i) begin
      ...
      if (l2_tlb_resp_valid_i && l2_tlb_resp_ready_o) begin
        resp_valid_q <= 1'b1; // Load new data
      end else if (axi_rready_i) begin
        resp_valid_q <= 1'b0; // Clear if AXI took it and no new data arriving
      end
    end
  end
